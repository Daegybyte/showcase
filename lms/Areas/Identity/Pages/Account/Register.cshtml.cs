...
namespace LMS.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        string CreateNewUser(string firstName, string lastName, DateTime DOB, string departmentAbbrev, string role)
        {
            ...
            //initialize return string and working list
            string newID = "";
            List<int> uids = new List<int>() { 0 };             // initialize list with "0" so it isn't empty

            // add uIDs from the Students, Professors, and Administrators tables as ints
            var tempUids = (from table in _lmsContext.Students
                                  select table.UId).ToArray();
            foreach (var n in tempUids)
            {
                uids.Add(int.Parse(n));
            }

            tempUids = (from table in _lmsContext.Professors select table.UId).ToArray();
            foreach (var n in tempUids)
            {
                uids.Add(int.Parse(n));
            }

            tempUids = (from table in _lmsContext.Administrators select table.UId).ToArray();
            foreach (var n in tempUids)
            {
                uids.Add(int.Parse(n));
            }

            // get the max int, add 1, and create new uid string
            newID = (uids.Max() + 1).ToString();

          
            // add new user row to appropriate table based on "role"
            if (role == "Student")
            {
                ...
                _lmsContext.Students.Add(newUser);
                var changed_rows = _lmsContext.SaveChanges();
            ...
            else if (role == "Professor")
            {
                ...
                _lmsContext.Professors.Add(newUser);
            ...
            else if (role == "Administrator")
            {
                ...
                _lmsContext.Administrators.Add(newUser);
                _lmsContext.SaveChanges();
            }
